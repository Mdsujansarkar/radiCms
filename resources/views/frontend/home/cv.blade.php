@extends('frontend.master')

@section('meta')

@foreach ($cvmetaTags as $meta_tag_single)
<meta content="{{ $meta_tag_single->meta_tag }}" name="keywords">
<meta content="{{ $meta_tag_single->site_tagline }}" name="description">
@endforeach

@endsection

@section('home')
<!--/ Intro Single star /-->
<section class="intro-single about">
    <div class="container">
       <div class="row">
          <div class="col-md-12 col-lg-8">
             <div class="title-single-box">
                <h1 class="title-single">দেলোয়ার হোসেন ফারুক সম্পর্কে</h1>
             </div>
          </div>
          <!-- 
             <div class="col-md-12 col-lg-4">
             <nav aria-label="breadcrumb" class="breadcrumb-box d-flex justify-content-lg-end">
             <ol class="breadcrumb">
             <li class="breadcrumb-item">
             <a href="#">Home</a>
             </li>
             <li class="breadcrumb-item active" aria-current="page">
             About
             </li>
             </ol>
             </nav>
             </div> -->
       </div>
    </div>
 </section>
 <!--/ Intro Single End /-->
 <!--/ Agent Single Star /-->
 <section class="agent-single">
    <div class="container">
    <div class="row">
       <div class="col-sm-12">
          <div class="row">
             <div class="col-md-5">
                <div class="agent-avatar-box">
                   <img src="{{ asset('frontend/assets/img/about-image.jpg') }}" alt="" class="agent-avatar img-fluid">
                </div>
             </div>
             <div class="col-md-7 section-md-t3">
                <div class="agent-info-box">
                   <!-- <div class="agent-title">
                      <div class="title-box-d">
                      <h3 class="title-d">Delowar Hossain Faruk</h3>
                      </div>
                      </div> -->
                   <div class="agent-content mb-3">
                      <p class="content-d color-text-a">
                         দেলোয়ার হোসেন ফারুক, তিনি সাধারণ ভাবে ফারুক নামে পরিচিত। তিনি কাজে কর্মে উদ্যমী  প্রকৃতির একজন যুবক। ফারুক দূরদর্শি সম্পন্ন  এবং সমূহ  সম্ভাব্যতায় কাজে পরিনত করিতে  পারেন এবং পরবর্তী কালের যে কোনো প্রকার ফলাফলে নিশ্চিত  হইতে পারেন। তিনি পরিস্থিতি  অনুযায়ী বিচক্ষণতার সাথে কাজে-কর্মে ব্যবস্থা  গ্রহণ করতে পারেন। তিনি বয়সে তরুণ কিন্তু নেতৃত্বদানে সফল একজন ব্যক্তি। তিনি একাধারে একজন সফল আইসিটি  উদ্যোক্তা, পরামর্শক, সমাজকর্মী, সংগঠক ও রাজনীতিবিদ। জাতির জনক বঙ্গবন্ধু শেখ মুজিবর রহমান এর আদর্শে অনুপ্রাণিত হয়ে ও মাননীয় প্রধানমন্ত্রী জননেত্রী শেখ হাসিনাকে আদর্শ ধরে তিনি তার রাজনৈতিক ও ব্যবসায়িক জীবন কে এগিয়ে নিয়ে যাচ্ছেন।<br>
                         তিনি বাংলাদেশ ছাত্রলীগের কেন্দ্রীয় দায়িত্ব পালনসহ বর্তমানে বাংলাদেশ আওয়ামীলীগ বিজ্ঞান ও প্রযুক্তি বিষয়ক কেন্দ্রীয় উপ-কমিটির সদস্য হিসাবে দায়িত্ব পালন করছেন। তিনি বাংলাদেশ এসোসিয়েশন অব সফ্টওয়্যার এন্ড ইনফরমেশন সার্ভিসেস (বেসিস) এর পরিচালক ছিলেন এবং বর্তমানে স্ট্যান্ডিং কমিটির চেয়ারম্যান হিসাবে দায়িত্ব পালন করছেন। ফারুক ঢাকা বিশ্ববিদ্যালয় থেকে স্বাস্থ্য অর্থনীতিতে স্নাতকোত্তর  ডিগ্রী অর্জনসহ ঢাকা কলেজ থেকে গণিতে স্নাতকোত্তর ডিগ্রী ও রয়েল ইউনিভার্সিটি থেকে ফাইন্যান্সে এমবিএ ডিগ্রী অর্জন করেন।<br><br>
                         আইসিটি শিল্পে ফারুকের আছে ১৪ বছরের অভিজ্ঞতা। তার প্রতিষ্ঠিত কোম্পানি  রেডিসন ডিজিটাল টেকনোলজিস্ লিমিটেড সফটওয়্যার উন্নয়নের কাজের সাথে জড়িত। রেডিসন আইটি বাংলাদেশের অন্যতম শীর্ষস্থানীয় আইটি পরামর্শক ও সফটওয়্যার প্রযুক্তি প্রদানকারী প্রতিষ্ঠান। চতুর্থ শিল্প বিপ্লবে বাংলাদেশ কে নেতৃত্ব দানকারী এই প্রতিষ্ঠানে রয়েছে এন.এল.পি, মেশিন লার্নিং, ব্লকচেইন, ডেটা সায়েন্স ও আই.ও.টি প্রযুক্তি প্রদানকারী বিশেষজ্ঞ টিম। সেই সাথে ওয়েবসাইট  ডিজাইন এবং উন্নয়ন, ওয়েব ভিত্তিক প্রতিষ্ঠান  উৎস পরিকল্পনা  <span id="dots">...</span><span id="more"> (ইআরপি ), সার্চ ইঞ্জিন  অপটিমাইজেশন (এসইও) , হাসপাতাল ব্যাবস্থাপনা  সফটওয়্যার, স্কুল ব্যবস্থাপনা সফটওয়্যার,  বিজনেস ইন্টিলিজেন্স  এবং বিভিন্ন প্রকার কার্যক্রম এর সঙ্গে যুক্ত। <br>
                         রেডিসন ডিজিটাল টেকনোলজিস লিমিটেড সাফল্যের সাথে ডিজিটাল বাংলাদেশ গড়ার ক্ষেত্রে দেশি ও বিদেশী প্রতিষ্ঠান এর সঙ্গে কাজ করে যাচ্ছে। বিভিন্ন বিদেশী প্রতিষ্ঠান এর সাথে কাজ করার সুবাদে ফারুক আমেরিকা, জার্মানি, ফ্রান্স, নেদারল্যান্ডস, বেলজিয়াম, ইতালি, সুইজারল্যান্ড, চীন, ভারত, জাপান, ইন্দোনেশিয়া, মালয়েশিয়া, সিঙ্গাপুর, ফিলিপাইন, নেপাল, যুক্তরাজ্য, অস্ট্রেলিয়া সহ বিশ্বের অনেক দেশ ভ্রমণ করেছেন এবং আইসিটি ভিত্তিক অনেক ইভেন্টে অংশ নিয়েছেন।<br>
                         রেডিসন ডিজিটাল টেকনোলজিস্ লিমিটেড ২০১৮ সালের বেসিস ন্যাশনাল আই সি টি অ্যাওয়ার্ডে চ্যাম্পিয়ন হবার গৌরব অর্জন করেছে । এছাড়াও ২০১৭ সালের আইসিটি এক্সপোতে ফ্রিল্যান্সারদের সম্মেলনে দেশ সেরা পুরস্কারসহ ১৫ টি পুরুস্কারের মধ্যে সাতটি পুরস্কারই পেয়েছে রেডিসন ডিজিটাল টেকনোলজিস্ লিমিটেড।
                         দেলোয়ার হোসেন ফারুক র‌েডিসন বিল্ডার্স অ্যান্ড হাউজিং ডেভলপমেন্ট লিমিটেড, র‌েডিসন মিডিয়া হাউস, রেডিসন ট্রেড ইন্টারন্যাশানাল, দয়াল বাবা ডট কম, বিনোদন ঘর প্রতিষ্ঠা করেন। তিনি দেশের অন্যতম জনপ্রিয় দৈনিক পত্রিকা দৈনিক আমাদের কাগজ এর প্রতিষ্ঠাতা। তিনি পত্রিকাটির সম্পাদক হিসাবেও প্রায় এক দশক ধরে দায়িত্ব পালন করছেন। তার বাবা একজন প্রখ্যাত সমাজকর্মী এবং তাঁর উপযুক্ত ছেলে হিসাবে তিনি তাঁর ভাল নামটি ধরে রেখেছিলেন। <br>
                         দেলোয়ার হোসেন ফারুক র‌েডিসন বিল্ডার্স অ্যান্ড হাউজিং ডেভলপমেন্ট লিমিটেড, র‌েডিসন মিডিয়া হাউস, রেডিসন ট্রেড ইন্টারন্যাশানাল, দয়াল বাবা ডট কম, বিনোদন ঘর প্রতিষ্ঠা করেন। তিনি দেশের অন্যতম জনপ্রিয় দৈনিক পত্রিকা দৈনিক আমাদের কাগজ এর প্রতিষ্ঠাতা। তিনি পত্রিকাটির সম্পাদক হিসাবেও প্রায় এক দশক ধরে দায়িত্ব পালন করছেন। তার বাবা একজন প্রখ্যাত সমাজকর্মী এবং তাঁর উপযুক্ত ছেলে হিসাবে তিনি তাঁর ভাল নামটি ধরে রেখেছিলেন। 
                         দেলোয়ার হোসেন ফারুক তার সামাজিক কাজ-কর্মের জন্যই অত্যধিক সুপরিচিত। তিনি বেসরকারী সংস্থা হিসাবে পরিবর্তন ফাউন্ডেশন ও বেটার টুমরো প্রতিষ্ঠা করেছিলেন। তিনি দেশের শিক্ষাব্যবস্থার উন্নয়নে অনেক অবদান রেখেছেন। তিনি কুমিল্লার আতাকরা উচ্চ বিদ্যালয় ও কলেজ এবং লাকসাম কম্পিউটার প্রশিক্ষণ ইনস্টিটিউট প্রতিষ্ঠা করেন।
                         </span> <button onclick="myFunction()" id="myBtn">Read more</button>
                      </p>
                   </div>
                </div>
             </div>
          </div>
       </div>
    </div>
 </section>
 <!--/ Agent Single End /-->
 <!--/ Intro Single star /-->
 <section class="intro-single about">
    <div class="container">
       <!--<div class="row">
          <div class="col-md-12 col-lg-8">
          <div class="title-single-box">
          <h1 class="title-single">Resume</h1>
          
          </div>
          </div>
          </div>-->
       <div class="row">
          <div class="col-md-12 col-lg-12">
             <!--  <img src="Delowar Hossain Faruk CV-3.jpg" alt="" class="img-b img-fluid">-->
             <!--<embed src="Delowar Hossain Faruk CV-3.pdf" width="920px" height="1250px" />-->
          </div>
       </div>
       <style>
          .resume-hedear{
          padding: 5px;
          margin: 0px 4px 5px 7px;
          border-radius: 5px;
          box-shadow: -1px 10px 11px 3px #8c8888;	
          background-color: rgb(20, 68, 90);
          color:#fff;
          }	
          .resume-edu{
          padding: 5px;
          margin: 0px 4px 5px 7px;
          border-radius: 5px;
          box-shadow: -1px 10px 11px 3px #8c8888;	
          background-color:  rgb(49, 72, 82);
          color:#fff;
          }
          .resume-politic{
          padding: 5px;
          margin: 0px 4px 5px 7px;
          border-radius: 5px;
          box-shadow: -1px 10px 11px 3px #8c8888;	
          background-color:  rgb(27, 56, 70);
          color:#fff;
          }
          .resume-social{
          padding: 5px;
          margin: 0px 4px 5px 7px;
          border-radius: 5px;
          box-shadow: -1px 10px 11px 3px #8c8888;	
          background-color:  rgb(11, 23, 29);
          color:#fff;
          }
          table tr td{
          font-family: 'HindSiliguri-Regular';
          font-size: 17px;
          line-height: 30px; 
          color:#fff;
          }
          .notification{
          background: #4c5cba;
          background: linear-gradient(90deg,rgb(86, 72, 72) 0%,rgb(112, 122, 128) 35%,rgb(68, 65, 70) 100%);
          }
       </style>
       <div class="row">
          <div class="col-md-12">
             <div id="printableArea" style="margin:0 auto width:1000px;background-color:white;">
                <ul class="notificationt" style="margin-top:10px;background-color:white;">
                <div class="resume-hedear">
                   <table>
                      <tr>
                         <td><span style="font-family: 'HindSiliguri-Bold'; font-size: 33px;line-height: 50px; font-weight: 600;">জীবনবৃত্তান্ত<br>
                            দেলোয়ার হোসেন ফারুক </span>
                         </td>
                      </tr>
                   </table>
                </div>
                <div class="resume-edu">
                   <table>
                      <tr>
                         <td><span style="font-family: 'HindSiliguri-Bold'; font-size: 30px;line-height: 45px; font-weight: 600;">
                            শিক্ষাগত যোগ্যতা
                            </span>
                         </td>
                         <td style="width:30px;font-family: 'HindSiliguri-Regular'; font-size: 33px;line-height: 50px; font-weight: 600;">:</td>
                         <td> <span style="margin-top:10px;font-family: 'HindSiliguri-Regular'; font-size: 16px;line-height: 30px; font-weight: 600;"> মাস্টার্স অব হেলথ্ ইকোনমিক্স, (স্বাস্থ্য অর্থনীতি), ঢাকা বিশ্ববিদ্যালয়;<br>
                            বি.এস.সি (অনার্স), এম.এস.সি. (গণিত); <br>
                            এমবিএ (ফাইন্যান্স) রয়েল ইউনিভার্সিটি।
                            </span>
                         </td>
                      </tr>
                   </table>
                </div>
                <div class="resume-politic">
                   <table>
                      <tr>
                         <td colspan="3"><span style="font-family: 'HindSiliguri-Bold'; font-size: 26px;line-height: 40px; font-weight: 600;">
                            রাজনৈতিক পরিচয়</span>
                         </td>
                      </tr>
                      <tr>
                         <td style="width:180px;">
                            <li>২০১৮ থেকে অদ্যবধি</li>
                         </td>
                         <td style="width:40px;">:</td>
                         <td>সদস্য, বাংলাদেশ আওয়ামীলীগ বিজ্ঞান ও প্রযুক্তি বিষয়ক কেন্দ্রীয় উপ-কমিটি।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>২০১৪-২০১৯</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক  সদস্য, বাংলাদেশ আওয়ামী স্বেচ্ছাসেবকলীগ কেন্দ্রীয় নির্বাহী সংসদ।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>২০১৮</li>
                         </td>
                         <td>:</td>
                         <td>বাংলাদেশ আওয়ামীলীগ জাতীয় নির্বাচন পরিচালনা কমিটি ২০১৮ এর আওতাধীন তথ্য ও প্রযুক্তি বিষয়ক কেন্দ্রীয় উপ-কমিটির সদস্য হিসাবে দায়িত্ব পালন।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>২০০৬ -২০১১</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক সহ-সম্পাদক, বাংলাদেশ ছাত্রলীগ কেন্দ্রীয় নির্বাহী সংসদ।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>২০০২-২০০৬</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক যুগ্ম-সাধারণ সম্পাদক, ঢাকা কলেজ ছাত্রলীগ ।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>২০০০ -২০০২</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক সদস্য, ঢাকা কলেজ ছাত্রলীগ।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>১৯৯৮ -২০০০</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক সদস্য,লাকসাম উপজেলা ছাত্রলীগ, কুমিল্লা।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>১৯৯৭ -১৯৯৯</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক আহ্বায়ক, নীলকান্ত ডিগ্রী কলেজ ছাত্রলীগ, মনোহরগঞ্জ, কুমিল্লা।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>১৯৯৪ -১৯৯৫</li>
                         </td>
                         <td>:</td>
                         <td>সাবেক সাধারণ-সম্পাদক, আশিরপাড় উচ্চবিদ্যালয় ছাত্রলীগ,মনোহরগঞ্জ,কুমিল্লা।</td>
                      </tr>
                   </table>
                </div>
                <div class="resume-social">
                   <table>
                      <tr>
                         <td colspan="3"><span style="font-family: 'HindSiliguri-Bold'; font-size: 26px;line-height: 40px; font-weight: 600;">
                            সামাজিক কর্মকান্ড</span>
                         </td>
                      </tr>
                      <tr>
                         <td style="width:180px;">
                            <li> চেয়ারম্যান</li>
                         </td>
                         <td style="width:40px;">:</td>
                         <td>বেসিস সদস্য কল্যাণ ও সদস্য সেবা স্থায়ী কমিটি (২০১৮-অদ্যবধি)।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>সাবেক পরিচালক</li>
                         </td>
                         <td>:</td>
                         <td> বেসিস (২০১৭-২০১৮)।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>চেয়ারম্যান</li>
                         </td>
                         <td>:</td>
                         <td>বেসিস সদস্য কল্যাণ স্থায়ী কমিটি (২০১৬-২০১৮)।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>প্রতিষ্ঠাতা ও সভাপতি</li>
                         </td>
                         <td>:</td>
                         <td>আতাকরা স্কুল এন্ড কলেজ, লাকসাম, কুমিল্লা।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>সাধারণ সম্পাদক	</li>
                         </td>
                         <td>:</td>
                         <td>ঢাকা কলেজ আবাহনী সমর্থক গোষ্ঠী (২০০০-২০০৪)।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>সাধারণ সম্পাদক</li>
                         </td>
                         <td>:</td>
                         <td>ঢাকা কলেজ বিতর্ক সংসদ (২০০০-২০০১)।</td>
                      </tr>
                      <tr>
                         <td>
                            <li>সাধারণ সম্পাদক</li>
                         </td>
                         <td>:</td>
                         <td>আতাকরা শেখরাসেল ক্রীড়াচক্র (১৯৯৪-১৯৯৮)।</td>
                      </tr>
                   </table>
                </div>
                <ul>
             </div>
          </div>
       </div>
    </div>
 </section>
 <!--/ Intro Single End /-->
 <!--/ News Grid Star /-->
 <section class="news-grid grid">
    <div class="container">
       <div class="row">
          <!--  </?html  while($view_political = $political->fetch_assoc()){ ?>
             <div class="col-md-4">
             <div class="card-box-b card-shadow news-box">
             <div class="img-box-b">
             <center><img src="images/political/</?html echo $view_political['image']; ?>" alt="" class="img-b img-fluid"></center>
             </div>
             
             <div class="card-overlays">
             <div class="card-header-bd">                
             <div class="card-title-p">
             <p class="title-2">
             <a href="political-single.html?polid=</?html echo $view_political['title_slug']; ?>"></?html echo mb_strimwidth($view_political['position_place'], 0, 75, "..."); ?></a>
             </p>
             <div class="card-category-b"><br>
             <a href="political-single.html?polid=</?html echo $view_political['title_slug']; ?>" class="category-b">» Read more</a>
             </div>
             </div>
             
             
             </div>
             </div>
             
             </div>
             </div>
             </?html } ?>  -->   
       </div>
    </div>
 </section>
 <!--/ News Grid End /-->
@endsection
            ...
            <div class="flex flex-wrap">
                ...
                <div class="w-full lg:w-full my-6 pr-0 lg:pr-2">
                    <div class="leading-loose">
                        <form class="p-10 bg-white rounded shadow-xl" action="{{ route('update.tag', $meta->id) }}"  method="post">
                            @csrf
                            <div class="mt-2">
                                <label class="block text-sm text-gray-600" for="title">{{ __('Page Title') }}</label>
                                <input class="w-full px-5  py-4 text-gray-700 bg-gray-200 rounded" id="title" name="title_tag" type="text" value="{{ $meta->title_tag }}">

                                <label class="block text-sm text-gray-600" for="title_one">{{ __('Meta Tag') }}</label>
                                <textarea class="w-full px-5  py-4 text-gray-700 bg-gray-200 resize rounded-md" name="meta_tag" id="title_one">{{ $meta->meta_tag }}</textarea>
                            ...
                            <div class="mt-2">
                                ...
                                <textarea class="w-full px-5  py-4 text-gray-700 bg-gray-200 resize rounded-md" name="site_tagline">{{ $meta->site_tagline}}</textarea>
                                    @if($errors->has('site_tagline'))
                                    <span class="block text-sm text-red-600"><i>{{ $errors->first('site_tagline'); }}<i></span>
                                    @endif
                            </div>
                            <div class="mt-6">
                                <button class="w-full px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded" type="submit">{{ __('Update') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
